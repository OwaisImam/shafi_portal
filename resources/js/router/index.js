...
const routes = [
    ...
    },
    {
        ...
        path: '/clients/create',
        meta: { title: 'Create Client', authRequired: true, },
    ...
    {
        path: '/users',
        meta: { authRequired: true, title: 'User List' },
    ...
    {
        path: '/error/403',
        meta: { authRequired: true, title: '403' },
...
router.beforeEach(async (routeTo, routeFrom, next) => {
    ...
    if (localStorage.getItem('accessToken') && routeTo.path == '/login') {
        next({ name: 'dashboard', query: { redirectFrom: routeTo.fullPath } });
    } else if (localStorage.getItem('accessToken') && routeTo.path != '/login') {
        next();
    } else {
