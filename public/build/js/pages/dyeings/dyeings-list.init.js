...
$('#newDyeingModal').on('shown.bs.modal', function () {
    $('#items-input').select2({
        dropdownParent: $("#newDyeingModal")
    });
...
// get json
getJSON("admin/department/" + department + "/dyeing", function (err, data) {
    if (err !== null) {
    ...
    $('#userList-table').DataTable({
        ...
        columns: [
            ...
            },
            {
                ...
                'bSortable': false,
                render: function (data, type, full) {
                    var hasEditPermission = datas.permissions.some(permission => permission.name === 'dyeing-edit');
                    var hasDeletePermission = datas.permissions.some(permission => permission.name === 'dyeing-delete');
                    if (hasDeletePermission || hasEditPermission) {
                        ...
                        if (hasEditPermission) {
                            actions += '<li><a href="#newDyeingModal" data-bs-toggle="modal" class="dropdown-item edit-list" data-edit-id="' + full.id + '"><i class="mdi mdi-pencil font-size-16 text-success me-1"></i> Edit</a></li>';
                        }

                        if (hasDeletePermission) {
                            actions += '<li><a href="#removeDyeingModal" data-bs-toggle="modal" class="dropdown-item remove-list" data-remove-id="' + full.id + '"><i class="mdi mdi-trash-can font-size-16 text-danger me-1"></i> Delete</a></li>';
                        }
...
var createContactForms = document.querySelectorAll('.createDyeing-form')
    ...
    form.addEventListener('submit', function (event) {
        if (!form.checkValidity()) {
            ...
            if (name !== "" && status !== "" && !editList) {
                ...
                var getEditid = 0;
                getEditid = document.getElementById('dyeingId-input').value;
...
function editContactList() {
    ...
    Array.from(document.querySelectorAll(".edit-list")).forEach(function (elem) {
        elem.addEventListener('click', function (event) {
            ...
            editList = true;
            document.getElementById("createDyeing-form").classList.remove("was-validated");
            itemListData.data = itemListData.data.map(function (item) {
                if (item.id == getEditid) {
                    document.getElementById("newDyeingModalLabel").innerHTML = "Edit Dyeing";
                    document.getElementById("addDyeing-btn").innerHTML = "Update";
                    document.getElementById("dyeingId-input").value = item.id;
                    ...
                    var form = document.getElementById('createDyeing-form');
...
// add list event
Array.from(document.querySelectorAll(".addDyeing-modal")).forEach(function (elem) {
    elem.addEventListener('click', function (event) {
        editList = false;
        document.getElementById("createDyeing-form").classList.remove("was-validated");
        document.getElementById("newDyeingModalLabel").innerHTML = "Add Dyeing";
        document.getElementById("addDyeing-btn").innerHTML = "Add";
        document.getElementById("switch6").checked = false;
        document.getElementById("dyeingId-input").value = "";
