...
// get json
getJSON("admin/department/" + department + "/items", function (err, data) {
    if (err !== null) {
    ...
    $('#userList-table').DataTable({
        ...
        columns: [
            ...
            },
            {
                ...
                'bSortable': false,
                render: function (data, type, full) {
                    var hasEditPermission = datas.permissions.some(permission => permission.name === 'items-edit');
                    var hasDeletePermission = datas.permissions.some(permission => permission.name === 'items-delete');
                    if (hasDeletePermission || hasEditPermission) {
                        ...
                        if (hasEditPermission) {
                            actions += '<li><a href="#newItemModal" data-bs-toggle="modal" class="dropdown-item edit-list" data-edit-id="' + full.id + '"><i class="mdi mdi-pencil font-size-16 text-success me-1"></i> Edit</a></li>';
                        }
...
var createContactForms = document.querySelectorAll('.createItem-form')
...
function editContactList() {
    ...
    Array.from(document.querySelectorAll(".edit-list")).forEach(function (elem) {
        elem.addEventListener('click', function (event) {
            ...
            editList = true;
            document.getElementById("createItem-form").classList.remove("was-validated");
            itemListData.data = itemListData.data.map(function (item) {
                if (item.id == getEditid) {
                    document.getElementById("newItemModalLabel").innerHTML = "Edit Department";
                    document.getElementById("addItem-btn").innerHTML = "Update";
                    ...
                    var form = document.getElementById('createItem-form');
...
// add list event
Array.from(document.querySelectorAll(".addItem-modal")).forEach(function (elem) {
    elem.addEventListener('click', function (event) {
        editList = false;
        document.getElementById("createItem-form").classList.remove("was-validated");
        document.getElementById("newItemModalLabel").innerHTML = "Add Item";
        document.getElementById("addItem-btn").innerHTML = "Add";
